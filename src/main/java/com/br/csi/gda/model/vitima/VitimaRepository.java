...
public interface VitimaRepository extends JpaRepository<Vitima, Long> {
    ...
    @Query(value = "SELECT usuario.id_usuario, usuario.uuid_usuario, usuario.nome, usuario.cpf, usuario.idade, usuario.data_cad, vitima.ultimo_end, vitima.descricao, vitima.presente, abrigo.id_abrigo, abrigo.uuid_abrigo, abrigo.nome_abrigo, abrigo.endereco FROM usuario, vitima, abrigo WHERE vitima.id_usuario = usuario.id_usuario AND abrigo.id_abrigo = usuario.id_abrigo AND abrigo.uuid_abrigo = :id", nativeQuery = true)
